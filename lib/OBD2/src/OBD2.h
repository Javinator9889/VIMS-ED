...
public:
  ...
  int pidRead(uint8_t mode, void* data, int length);
  int pidRead(uint8_t mode, uint8_t pid, void* data, int length);
  int clearStoredDTC(void);

private:
  int supportedPidsRead();
  int pidReadStub(uint8_t mode, float pid, void *data, int length);
  int pidWrite(uint8_t mode, float pid);
