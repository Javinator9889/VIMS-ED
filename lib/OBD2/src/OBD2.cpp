...
int OBD2Class::pidRead(uint8_t mode, void* data, int length)
{
  return pidReadStub(mode, NAN, data, length);
}

int OBD2Class::pidRead(uint8_t mode, uint8_t pid, void* data, int length)
{
  return pidReadStub(mode, pid, data, length);
}

int OBD2Class::clearStoredDTC(void)
{
  return pidWrite(0x04, NAN);
}

int OBD2Class::pidWrite(uint8_t mode, float pid)
{
  ...
  for (int retries = 10; retries > 0; retries--) {
    ...
    CAN.write(mode);
    // this functions is private and can only be called from either
    // "pidRead" (without PID) or "pidRead" (with PID as uint8_t). If
    // the first function calls us, then NAN is given. Otherwise, we
    // know it is an uint8_t casted as float.
    if (pid != NAN)
      CAN.write((uint8_t) pid);
...
int OBD2Class::pidReadStub(uint8_t mode, float pid, void* data, int length)
{
  if (!pidWrite(mode, pid))
    return 0;
